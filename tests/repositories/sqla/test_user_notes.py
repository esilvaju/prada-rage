...
def test_crud_user_notes(db_session, fake: Faker):
    ...
    note = SQLAUserNote(title=note_title, content=fake.text(), user=maany, type=NoteType.USER)
...
def test_delete_notes(db_session, fake: Faker):
    ...
    maany = SQLAUser(
        ...
        notes=[
            SQLAUserNote(title="Test Note", content="This is a test note", type=NoteType.USER),
            SQLAUserNote(title=note_title, content="This is a test note 2", type=NoteType.USER),
        ],
    )
...
def test_user_is_present_in_notes(db_session, faker: Faker):
    ...
    maany = SQLAUser(
        prada_user_uuid=username,
        notes=[
            SQLAUserNote(title=note_title, content="This is a test note", type=NoteType.USER),
            SQLAUserNote(title=faker.name(), content="This is a test note 2", type=NoteType.USER),
        ],
    )
